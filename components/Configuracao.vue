<template>
  <section class="bg-[#131619] min-h-screen font-jakarta flex flex-col items-center">
    ...
    <div class="flex flex-col w-1/3">
      ...
      <!-- Campo para Data de Aniversário -->
      <div class="flex flex-col my-4">
      ...
      <div class="flex flex-col my-4">
        ...
        <input type="url" v-model="url" name="link" id="link" class="border-[#D9D9D9] border bg-transparent p-2 rounded-md text-[#C9C9C9]" />
        <div class="mt-3">
          <button type="button" class="text-white bg-transparent border border-[#D9D9D9] p-2 rounded-md hover:bg-[#cccccc1f] duration-100 transition-all">Adicionar URL</button>
        </div>
      </div>
      <!-- Botões de Ação -->
      <div class="mb-5 mt-8">
        <button v-if="isEditing" type="button" @click="atualizarPerfil" class="text-black bg-[#D9D9D9] p-4 font-semibold rounded-lg hover:bg-[#919191] duration-200 transition-all">Atualizar Perfil</button>
        <button v-else type="button" @click="cadastrarPerfil" class="text-black bg-[#D9D9D9] p-4 font-semibold rounded-lg hover:bg-[#919191] duration-200 transition-all">Cadastrar Perfil</button>
      </div>
    </div>
  </section>
</template>
...
const isEditing = ref(false); // Inicialmente falso
...
const carregarPerfil = async () => {
  try {
    const response = await $fetch("/api/users/get.perfil", {
      method: "GET",
    });

    if (response?.data) {
      isEditing.value = true; // Define como edição se os dados existirem
      const perfil = response.data;
      nome.value = perfil.nome;
      dataAniversario.value = perfil.dataAniversario;
      localizacao.value = perfil.localizacao;
      bio.value = perfil.bio;
      url.value = perfil.url;
      imagePreview.value = perfil.foto || null; // Exibe a foto existente, se houver
    }
  } catch (error) {
...
const cadastrarPerfil = async () => {
  ...
  try {
    ...
    const response = await $fetch("/api/users/create.post.perfil", {
      method: "POST",
      body: formData,
    });
    ...
    alert("Erro ao criar perfil. Verifique os campos e tente novamente.");
  }
...
const atualizarPerfil = async () => {
  ...
  try {
    const response = await $fetch("/api/users/update.perfil", {
      method: "PUT",
      body: formData,
    });
